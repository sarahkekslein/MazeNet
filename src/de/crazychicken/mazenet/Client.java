...
public class Client {
	...
	public void play() throws UnmarshalException, IOException {
		...
		while (true) {
			...
			// Aktion je nach Typ der Nachricht ausfÃ¼hren
	...
	private void onAwaitMove(AwaitMoveMessageType awaitMoveMessage) throws UnmarshalException, IOException {
		...
		this.moveCount++;
		// schauen, dass selbst nicht ganz eingekesselt und möglichst wenige
		...
		if (message.isAccept()) {
			...
			if (err == ErrorType.ILLEGAL_MOVE) {
				...
				if (foundTreasure) {
					...
					// anderen Zug gewählt werden und nicht wieder derselbe
					// berechnet werden
					useRandom = true;
				}
	...
	private void doShift(BoardType board, CardType shiftCard, PositionType shiftPosition) {
		...
		if (row == 0 && column % 2 == 1) { // nach unten
			...
			// Position ist keine gÃ¼ltige Schiebeposition
